...
export default function GameSessionPage() {
  ...
  const [prepSeats, setPrepSeats] = useState<{ player_id: number; seat_number: number }[]>([])
  const [eveningPlayers, setEveningPlayers] = useState<{ id: number; player: { id: number; nickname: string } }[]>(
    []
  )
  ...
  const load = useCallback(async () => {
    ...
    setS(data)
    const incomplete = data.voting_rounds.filter((v) => !v.completed).sort((a, b) => a.index - b.index)[0]
  ...
  useEffect(() => {
    ...
    if (s.players.length) {
      setPrepSeats(s.players.map((p) => ({ player_id: p.player, seat_number: p.seat_number })))
    }
  }, [s, prepSeats.length])
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const ep = await apiJson<{ id: number; player: { id: number; nickname: string } }[]>(
  ...
  async function patchSession(body: Record<string, unknown>) {
    setErr('')
    try {
      await apiJson(`/sessions/${id}/`, { method: 'PATCH', body: JSON.stringify(body) })
      await load()
    } catch (e) {
      setErr(e instanceof Error ? e.message : 'Ошибка')
    }
  ...
  return (
    <div className="shell" style={{ maxWidth: 800, paddingTop: 0 }}>
      ...
      <header style={{ marginTop: 8 }}>
        <Link to={`/evening/${s.evening_id}`} className="muted" style={{ fontSize: '0.85rem' }}>
          ← К вечеру
        </Link>
      ...
      <section className="card stack" style={{ marginTop: 10 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            ...
            <tbody>
              {[...s.players].sort((a, b) => a.seat_number - b.seat_number).map((p) => (
                <tr key={p.id} className={p.eliminated || p.excluded_by_fouls ? 'eliminated' : undefined}>
                  <td>{p.seat_number}</td>
                  <td>{p.nickname}</td>
                  <td>
                    {s.stage === 'post_edit' && !me?.is_staff ? (
                      ...
                      <button type="button" className="btn" onClick={() => void call('foul/', { seat_number: p.seat_number })}>
                        +фол ({p.fouls})
                      </button>
                    )}
                  </td>
                  ...
                  <td>
                    {s.stage === 'post_edit' ? (
                      <select
                        value={p.role}
                        onChange={(e) => void patchPlayer(p.id, { role: e.target.value })}
                      >
                        {ROLES.map((r) => (
        ...
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          {s.players.length === 0 && eveningPlayers.length > 0 && (
            <button
              ...
              onClick={() =>
                setPrepSeats(
                  eveningPlayers.map((ep, i) => ({ player_id: ep.player.id, seat_number: i + 1 }))
                )
              }
            >
          ...
          {prepSeats.map((row, idx) => (
            <div key={idx} className="row">
              <span>
                {eveningPlayers.find((e) => e.player.id === row.player_id)?.player.nickname ||
                  `Игрок ${row.player_id}`}
              </span>
              <input
                type="number"
          ...
          <button type="button" className="btn btn-primary" onClick={() => void applyPrep()}>
            Сохранить состав
          </button>
      ...
      {(s.stage === 'voting' || s.stage === 'revoting') && (
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          <label>
            ...
            <input value={nomText} onChange={(e) => setNomText(e.target.value)} />
          </label>
          {nomText
            .split(/[,\s]+/)
            .map((x) => parseInt(x.trim(), 10))
            ...
            .map((seat) => (
              <label key={seat}>
                ...
                <input
                  ...
                  onChange={(e) => setVoteParts((v) => ({ ...v, [String(seat)]: e.target.value }))}
                />
              </label>
            ))}
          <div className="row">
      ...
      {s.stage === 'lift_pending' && (
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          <button type="button" className="btn btn-primary" onClick={() => void call('lift/', { eliminate_all: true })}>
            Поднятие (все выбывают)
          </button>
          <button type="button" className="btn" onClick={() => void call('lift/', { eliminate_all: false })}>
      ...
      {s.stage === 'shooting' && (
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          <label>
            <input type="checkbox" checked={shootMiss} onChange={(e) => setShootMiss(e.target.checked)} /> Промах (X)
          </label>
          {!shootMiss && (
            <label>
              Место цели
              <input value={shootTarget} onChange={(e) => setShootTarget(e.target.value)} />
            </label>
          )}
          <div className="row">
            <button
        ...
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          <div className="row">
            <button
              ...
              onClick={() =>
                void call('testament/', {
                  seats: testSeats.map((x) => parseInt(x, 10)).filter((x) => !Number.isNaN(x)),
                })
              }
            >
              Сохранить
      ...
      {s.stage !== 'prep' && s.stage !== 'post_edit' && (
        <section className="card stack" style={{ marginTop: 12 }}>
          ...
          <label>
            ...
            <input value={completeWord} onChange={(e) => setCompleteWord(e.target.value)} />
          </label>
          <button
            type="button"
            className="btn btn-primary"
      ...
      {s.stage === 'post_edit' && (
        <section className="card stack" style={{ marginTop: 12 }}>
          <strong>Итог</strong>
          <select value={s.winner} onChange={(e) => void patchSession({ winner: e.target.value })}>
            <option value="">—</option>
            {WINNERS.map((w) => (
